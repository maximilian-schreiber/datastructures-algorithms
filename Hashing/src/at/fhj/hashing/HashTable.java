...
public class HashTable {
	...
	public int insert(Object obj) {
		...
		if (getIndexFromKey(key) >= 0){
			return -1;
		}

		int tryAmount = 1;
		int currentIndex = hashFunction1(key);

		while(!isEmpty(currentIndex)){
			currentIndex = getProbing(key, tryAmount++);
			if(tryAmount == table.length){
				return -1;
			}
		}

		setEntry(currentIndex, obj);
		return currentIndex;
	}
	...
	public int insertBrent(Object obj) {
		...
		while(!isFree(currentIndex)){
			Object oldValue = getEntry(currentIndex);
			int probingCurrent = getProbingBrent(currentIndex, key);
			int probingOld = getProbingBrent(currentIndex, oldValue.hashCode());

			if (isFree(probingCurrent) || !isFree(probingOld)) {
				currentIndex = probingCurrent;
			} else {
				setEntry(probingOld, oldValue);
	...
	private int getProbing(int key, int tryAmount){
		int result = hashFunction1(key)-tryAmount*hashFunction2(key);

		if(result < 0){
			result = (result+tryAmount*table.length)%table.length;
		}

		return result;
	}

	private int getProbingBrent(int key, int key2){
		int result = hashFunction1(key)-hashFunction2(key2);
	...
	private int getIndexFromKey(int key) {
		...
		while (isFree(currentIndex) || getEntry(currentIndex).hashCode() != key) {
			currentIndex = getProbing(key, tryAmount++);
			if (tryAmount == table.length) {
				return -1;
			}
